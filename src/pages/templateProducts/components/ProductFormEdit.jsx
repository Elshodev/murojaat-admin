...
function ProductFormEdit({ data, datas = [], branches = [] }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    request(`/products/template/${data.id}`, "PUT", formData)
      .then(() => {
        showToast.success("Шаблонные продукт успешно обновлена!");
        queryClient.invalidateQueries([`/products/template${data.id}`]);
  ...
  const handleDelete = () => {
    setLoading(true);
    request(`/products/template/${data.id}`, "DELETE")
      .then(() => {
        showToast.success("Шаблонные продукт успешно удалена!");
        navigate(`/templateProducts`);
      })
