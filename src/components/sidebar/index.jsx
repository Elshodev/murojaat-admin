    ...
    <>
      <div className="fixed left-0 top-0 z-[1] flex h-[80px] w-full items-center justify-between bg-white px-8 lg:hidden xs:px-4">
        <Link to={"/"} className="block">
          <img
            className="h-[45px] w-[100%] object-cover"
            src={logo}
            alt="panel"
          />
        </Link>
      ...
      <div
        className={`scroll-hiddenfixed p-2 lg:sticky bottom-0 flex-col overflow-auto rounded-none border border-[#EEE] bg-white  shadow-none transition-all md:top-[80px] md:z-[111] md:flex lg:bottom-auto lg:top-0 lg:flex lg:h-screen max-w-[260px] w-full shrink-0 top-[80px] z-[20] flex ${
          isShow ? "md:left-0 left-0" : "md:-left-full -left-full"
        }`}
      >
        <div className="h-[60px] mb-2 flex items-center justify-center">
          <Link to={"/"} className="flex justify-center ">
            <img className="w-[80%]" src={logo} alt="panel" />
          </Link>
        </div>
